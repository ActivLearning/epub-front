...
export default {
    ...
    methods: {
        ...
        copy() {
            ...
            let result = iframe.contentWindow.document.execCommand(
                'copy',
                false,
                null
            )
        ...
        getSelectionText() {
            ...
            this.selectedText =
                sel.anchorNode &&
                sel.anchorNode.data.substring(sel.baseOffset, sel.extentOffset)
        },
        fullscreen() {
            // 进入全屏模式
        ...
        setStyle(fontSize) {
            ...
            this.book.setStyle(
                'background-color',
                this.themes[this.options.theme].bgColor
            )
        },
        isMarked() {
            const result = this.notes.filter(
                note => note.selectedText === this.selectedText
            )
        ...
        setProgress() {
            ...
            this.progress =
                this.locations.percentageFromCfi(currentLocationCfi) * 100

            console.log(this.progress)
        },
        clearStorage() {
            let clear = confirm(
                'Clearing the browser cache will delete all bookmarks, annotations and notes'
            )
            if (clear) {
